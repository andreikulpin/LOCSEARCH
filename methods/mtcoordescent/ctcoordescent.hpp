...
#ifndef CTCOORDESCENT_HPP
#define CTCOORDESCENT_HPP
...
namespace LOCSEARCH {
     ...
     */
    template <typename FT> class CTCoordinateDescent : public COMPI::Solver<FT> {
    public:
         ...
         */
        CTCoordinateDescent(const COMPI::MPProblem<FT>& prob) :
        mProblem(prob) {
        ...
        bool search(FT* x, FT& v) override {
            ...
            while (true) {
                // begin parallel loop
                auto subloop = [&] (int num, int size) {
                    for (int i = num; i < n2; i += size) {
                        xx[i] = xold;
                        const int I = i / 2;
                        FT xi;
