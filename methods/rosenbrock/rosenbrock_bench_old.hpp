...
/* 
 * File:   rosenbrock_bench.hpp
 * Author: andrei
 *
 * Created on May 2, 2018, 7:09 PM
 */

#ifndef ROSENBROCK_BENCH_OLD_HPP
#define ROSENBROCK_BENCH_OLD_HPP
...
namespace LOCSEARCH {
     ...
     */
    template <typename FT> class RosenbrockBenchMethodOld : public COMPI::Solver<FT> {
    public:
         ...
         */
        RosenbrockBenchMethodOld(const COMPI::MPProblem<FT>& prob, FT globMin) : mProblem(prob), 
                mGlobMin(globMin) 
        {
        ...
        bool search(FT* x, FT& v) override {
            ...
            while (!br) {
                ...
                if (stageNum >= mOptions.maxStepsNumber) {
                    br = true;
                    if (mOptions.mDoTracing) {
