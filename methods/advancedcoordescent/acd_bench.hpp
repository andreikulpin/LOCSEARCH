 ...
 * Author: kulpin
 *
 * Created on April 26, 2018, 11:46 PM
 */
...
namespace LOCSEARCH {
     ...
     */
    template <typename FT> class BenchmarkAdvancedCoordinateDescent : public COMPI::Solver<FT> {
    public:
        ...
        struct Options {
             ...
             * Searh type
             */
            SearchTypes mSearchType = SearchTypes::NO_DESCENT;
            /**
             * Adaptation type
             */
            VicinityAdaptationPolicy mVicinityAdaptation = VicinityAdaptationPolicy::UNIFORM_ADAPTATION;
            /**
             * Trace on/off
             */
         ...
         */
        BenchmarkAdvancedCoordinateDescent(const Benchmark<FT>& prob) :
        mBenchmark(prob) {}
        ...
        bool search(FT* x, FT& v) override {
            ...
            int n = mBenchmark.getDim();
            
            const snowgoose::Box<FT>& box = *(new snowgoose::Box<FT>(n));
            for (int i = 0; i < n; i++) {
                box.mA[i] = mBenchmark.getBounds()[i].first;
                box.mB[i] = mBenchmark.getBounds()[i].second;
            }
            
            snowgoose::BoxUtils::project(x, box);
            std::vector<FT> xVector(n);
            
            xVector.assign(x, x + n);
            FT fcur = mBenchmark.calcFunc(xVector);
            ...
            auto step = [&] () {
                ...
                for (int i = 0; i < n;) {
                    ...
                    x[i] = y;
                    xVector.assign(x, x + n);
                    const FT fn = mBenchmark.calcFunc(xVector);
                    const FT dx = y - tmp;
